...
CREATE TABLE IF NOT EXISTS public.driver_accounts (
    ...
    parent_phone TEXT,
    parent_email TEXT,
    safety_score NUMERIC DEFAULT 100.0 CONSTRAINT safety_score_range CHECK (safety_score >= 0.0 AND safety_score <= 100.0),
    clean_trips INT DEFAULT 0 CONSTRAINT clean_trips_positive CHECK (clean_trips >= 0),
    total_trips INT DEFAULT 0 CONSTRAINT total_trips_positive CHECK (total_trips >= 0),
    total_distance_miles NUMERIC DEFAULT 0.0 CONSTRAINT distance_positive CHECK (total_distance_miles >= 0.0),
    points INT DEFAULT 0,
    level INT DEFAULT 1 CONSTRAINT level_positive CHECK (level >= 1),
...
CREATE TABLE IF NOT EXISTS public.road_hazards (
    ...
    lat DOUBLE PRECISION NOT NULL CONSTRAINT lat_bounds CHECK (lat >= -90.0 AND lat <= 90.0),
    lng DOUBLE PRECISION NOT NULL CONSTRAINT lng_bounds CHECK (lng >= -180.0 AND lng <= 180.0),
    upvotes INT DEFAULT 1 CONSTRAINT upvotes_bounds CHECK (upvotes >= 1 AND upvotes <= 100000),
